...
const useBooking = () => {
    const [bookingList, setBookingList] = useState<BookingRequest[]>();
    ...
    const runSaveBookking = async () => {
        ...
        setBooking((prev) => ({ ...prev!, customer: responseCustomer.data, seat: responseSeat.data, billboard: responseBillboard.data }));
    ...
    const runGetBookingList = async () => {
        ...
        const datosAjustados = response.data.map((prev) => ({ ...prev!, customer: prev.customer, seat: prev.seat, billboard: prev.billboard }));
        if (response.statusText === 'OK') { setBookingList(datosAjustados) }
        else { alert('Algo paso, intente nuevamente') }
